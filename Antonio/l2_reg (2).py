...
@app.cell
def _(SummaryWriter, mo):
    l2_lambda = mo.cli_args().get("l2_lambda") or 0.001

    writer = SummaryWriter(f"runs/l2_reg_{l2_lambda}")
...
def _(device, evaluate, l2_regularization, torch, writer):
    def train(model, optimizer, loss_fn, train_loader, val_loader, num_epochs, l2_lambda):
...
@app.cell
def _(l2_lambda, loss_fn, model, optimizer, train, train_loader, val_loader):
    train(model, optimizer, loss_fn, train_loader, val_loader, num_epochs=300, l2_lambda=l2_lambda)
    return
